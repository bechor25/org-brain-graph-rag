...
**צמתים מובנים (מ-load, ללא LLM):** `WorkItem{type: Issue|Story|Bug|Epic|Task}`, `Test`, `TestExecution`, `TestRun`, `Document{kind: KIP|Page}`, `Person`, `Component`, `Version`, `Sprint`, `Commit`, `PullRequest`, `File`, `Chunk`, `IndexMeta` (מודל/מימד/ספירה של כל vector index), `StatusChange` (צומת-אירוע מה-changelog: field, from, to, at).

**קשתות מובנות (הסט הסגור, עודכן 2026-09-03 אחרי load):** `ASSIGNED_TO{valid_from,valid_to,source}`, `REPORTED_BY`, `AUTHORED`, `COMMENTED{at}`, `MENTIONS_PERSON`, `IN_COMPONENT`, `FIX_VERSION`, `AFFECTS_VERSION`, `PARENT_OF`, `LINKS_TO{type,raw_type}` (אוצר מילים: relates/duplicates/blocks/causes/clones/supersedes/splits/depends_on/tests/executes/defect), `REFERENCES{via,kinds}`, `TESTS`, `IN_PLAN`, `EXECUTED_IN`, `HAS_RUN{status,reason}`, `IN_SPRINT`, `IN_SPACE`, `TOUCHES` (Commit→File), `RESOLVES` (Commit→WorkItem), `IMPLEMENTS_KIP` (Commit→Document), `HAS_COMMIT` (PullRequest→Commit), `CHILD_OF`, `VARIANT_OF` (Document→Document), `HAS_CHUNK`, `HAS_CHANGE` (WorkItem→StatusChange), `SAME_AS` (לפני merge סופי). צמתים מובנים נוספים: `PullRequest`, `File`, `Space`. `TestPlan`/`TestSet` הם WorkItems (מפתחות XP/XS); containers מסוג testplan/testset אינם נטענים.

**צמתים/קשתות מחילוץ LLM (מ-extract, מונחה-סכמה):** `Entity{kind: Feature|Decision|Problem|Alternative|Risk|Technology}`; `MENTIONS` (Chunk→Entity, וגם Chunk→Document|WorkItem|Component כששם הישות תואם מפתח קיים; עם quote, kind, description), `DECIDES` (Document|WorkItem→Decision), `MOTIVATED_BY` (Decision|Feature→Problem), `REJECTS` (Decision|Document|WorkItem→Alternative, reason), `IMPLEMENTS` (WorkItem|Commit|Document→Feature), `DEPENDS_ON` (Component|Feature|Technology|Decision→Component|Feature|Technology|Document), `INTRODUCES_RISK` (Decision|Feature|Problem|Alternative→Risk). הטבלה המחייבת בקוד: `brain/extract/shapes.json`. (צורות עודכנו 2026-09-07 אחרי extract: 406 MOTIVATED_BY מ-Feature ו-235 DEPENDS_ON בין טכנולוגיות הם דפוס אמיתי בטקסט, לא שגיאה.) `weak=true` על Decision ללא MOTIVATED_BY/REJECTS — אינפורמטיבי.
