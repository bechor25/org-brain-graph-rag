...
**צמתים/קשתות מחילוץ LLM (מ-extract, מונחה-סכמה):** `Entity{kind: Feature|Decision|Problem|Alternative|Risk|Technology}`; `MENTIONS` (Chunk→Entity, וגם Chunk→Document|WorkItem|Component כששם הישות תואם מפתח קיים; עם quote, kind, description), `DECIDES` (Document|WorkItem→Decision), `MOTIVATED_BY` (Decision|Feature→Problem), `REJECTS` (Decision|Document→Alternative, reason), `IMPLEMENTS` (WorkItem|Commit→Feature), `DEPENDS_ON` (Component|Feature|Technology→Component|Feature|Technology|Document), `INTRODUCES_RISK` (Decision|Feature|Problem→Risk). (צורות עודכנו 2026-09-07 אחרי extract: 406 MOTIVATED_BY מ-Feature ו-235 DEPENDS_ON בין טכנולוגיות הם דפוס אמיתי בטקסט, לא שגיאה.) `weak=true` על Decision ללא MOTIVATED_BY/REJECTS — אינפורמטיבי.
