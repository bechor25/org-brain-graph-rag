...
- **הקפאת snapshot להערכה:** מרגע בניית batches של תשובות ועד merge השופט אין להריץ `brain eval run` (גם לא `--force`) ואין לשנות את הגרף; שינוי גרף (אינקרמנטלי) = מדידה נפרדת עם snapshot משלה.
- **snapshot של מדידה = sha של קוד + census של גרף** (ספירת צמתים/קשתות/קהילות בזמן האחזור); בדיקה שרצה על גרף אחר מדווחת `in_retrieval_snapshot` לצד `in_graph_now`, לא "לא בגרף".
- **dedupe של שיפוט חופף הוא per-aggregate:** כל טבלה שמסכמת מקרים/זוגות חופפים קובצת לפי `case_id`/`pair_id` לפני ספירה; אי-הסכמה pairwise מפורסמת ליד הסכמת המדדים.
