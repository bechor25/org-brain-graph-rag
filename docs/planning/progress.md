...
**כל ארבע התכניות סגורות (2026-09-17).** Plan 1 שער 12/13 (FAIL מקובל יחיד: recall אנשים 0.779), `make smoke` ירוק על DB שקט; Plan 2 שער עבר (19/19, 99% ציטוטים); Plan 3: `docs/report/eval-report.md` מלא — מטריצה 7×5 בשתי שכבות, שופט עיוור (229 שיפוטים, הסכמה 90% exact), אינקרמנטלי (10 issues ב-39.7s), שני הסעיפים הפרשניים נכתבו. ריפו ציבורי: https://github.com/bechor25/org-brain-graph-rag · 2,431 בדיקות · 16 שיעורים ב-`docs/lessons/`.
...
| 2 מצב A + שכבה 2 | ✅ (38cbbb9) — **המטריצה הראשונה** | 38cbbb9 | 32 שאלות × 7 אסטרטגיות = 224 ריצות, rerun = 224 unchanged; recall של gold_evidence: **S3 0.23** (precision 0.56, hit@10 0.81) · S6 0.56 על 8 שאלות שנקשרו → **אחרי תיקון (8a2474f, d5ebc44): 18/32 נקשרות** (KIP = היסטוריה נגזרת מ-commits מיישמים + issues מפנים, `props.via`), 14 n/a כנות (אין מפתח/תאריך/גרסה); סדר טוטאלי בכל שאילתת זמן (2e3c42c); **ממצא:** gold של cq13/cq19 נגזר דרך `changes_between` לא-ממוין → מדד סדר-אחסון (recall 0.83 היה מקרי) — gold נגזר מחדש בקוד אחרי התיקון · S5 0.19 על גלובלי · S4 0.15 (15 ms, 1.3k tokens) · S1 0.08 / S1+rerank 0.08 · S2 0.05; **reranker: hit@1 0.125→0.156 בלבד, ×13 latency (137→1,774 ms p50)**; S1/S2 שורפים 3.8k tokens ב-precision 0.09–0.11; cross-lingual: S3/S4/S6 אינווריאנטים לשפה (Jaccard 1.0 ב-3/4 זוגות), S1/S2 סוטים; S5 מחוץ לתבנית: recall ≤0.12 — כלל ה-n/a מוצדק במדידה; שני recall מפורסמים: `recall` (chunk שה-parent שלו הוא צומת gold נספר) ו-`recall_strict` — S1 0.077 מול 0.014, השאר זהים |
| 2b ממצאים אחרי הסוויפ | 🟡 | | (א) `brain eval run --strategies s6` דרס את הדוח לעמודה אחת (`stale: false` על הכל) → תיקון: הדוח נבנה מכל קובצי הריצה, stale לפי עמודה; (ב) **0/32 שאלות על בסיס `synthetic_truth.json`** — 4 מסלולי truth נדגמו, 3 נארזו כ-spare ולא נשאלו, q104 יצא עודף → זרוע ה-truth של הכרעה 1 לא מומשה; הכרעה: לא לשנות את ה-32 באמצע ריצת התשובות; לתעד כפער ידוע בדוח ולהוסיף 4 שאלות truth כ-supplement אחרי merge השופט אם יש זמן |
| 3 תשובות + שופט עיוור | ✅ **שכבה 3 נמדדה** — 229 שיפוטים על 190 מקרים + 143 pairwise, 0 נדחו | 4da025c, 50fb3fb | **faithfulness 2.00 בכל אסטרטגיה קבועה** (הכשלים = אחזור, לא הזיה; טענה לא נתמכת אחת ב-190); **correctness: agentic 1.40 · s5 1.00 · s4 0.95 · s6 0.67 · s1r 0.52 · s1 0.50 · s2 0.50 · s3 0.47**; pairwise מול baseline s1r: s4 62.5% נצחונות · s5 85.7% · s3 27.8% · s6 30% · s2 10%; `both_wrong` ~40% מהזוגות; הסכמה בין שופטים (39 חופפים): correctness exact 89.7% / within-1 100%; cross-check ציטוטים קוד-מול-שופט 182/190 (8 אי-הסכמות, **אף אחת אגנטית**: 7 "לא בגרף" — 3 מהן מזהי קהילה שנמחקו ב-re-partition האינקרמנטלי בין האחזור (09:35) לבדיקה (10:13), כלומר הקוד בדק מול גרף שזז — ו-1 בלי ציטוט; המזהים המקוצרים במצב B הורידו citation_validity ל-1 אבל אינם "קיצוניים" ולא נספרים); **S3 מוביל ב-recall אבל אחרון ב-correctness** — מחזיר ישויות בלי הפריט; S4/S5 מחזירים שורות/דוחות שהעונה יכול להשתמש בהן; agentic (מצב B) הכי נכון — אבל בלי faithfulness מדיד (אין הקשר ארוז) ובעלות של 5–7 קריאות כלי |
| 4 אינקרמנטלי | ✅ (b5a0dba…4cf28f9) `validate_incremental` = COMPLETE; reset לא הופעל (הפרוסה נשארת בגרף) | b5a0dba, 81a2010, 8e3957a, 4cf28f9 | 10 issues של 2026 דרך 12 שלבים ב-**39.7s** (harvest 1.8 · canon 10.4 · load 8.6 · chunk 7.6 · build 1.3 · merge 0.9 · resolve 2.4 · communities 5.1 · stamp 0.5 · index 0.9); +144 צמתים/+239 קשתות; בסיס byte-identical 5/5; **באג שנחשף ותוקן:** merge חלקי דרס provenance (`SET n += list`) — עכשיו union + batch id `<slice>/shard-NN/NNN`; 3 ישויות בסיס שוחזרו מה-.out.json המקורי; **Leiden = re-partition גלובלי:** 645/1,297 hashes, 62 דוחות אבדו, כיסוי 89%→47.5% — נמדד, לא תוקן; **5 שאלות: מצב A 3/5, מצב B 5/5** (70/70 ציטוטים); בנק ה-Cypher עיוור ל-Resolved (9/10 החדשים); KAFKA-20043 בלי קשתות Entity יוצאות (Risk תלוי ב-chunk) → S3 מפספס; `global_search` לא מגיע לפרוסה (אין סיכום מחדש); rollback dry-run: 43 chunks/10 WI/59 SC/3 Person/26 Entity |
| 5 דוח + שער | ✅ **שער Plan 3 עבר** (2026-09-17) | 0b7e4f6, 691e708, c2ce809 | `brain eval report` מ-9 קובצי JSON עם sha/stale לכל קלט; "מתי מה" נגזר: **4/5 סוגים — מובילת ה-recall ≠ מובילת ה-correctness** (agentic מובילה ב-correctness ב-4, S4 בהשפעה); pairwise מתוקן לפי pair_id: **s4 65.6% נצחונות מול baseline** (verdict מפוצל = tie), s5 83.3%, s6 33.3%, s3 25%, s2 9.4%; הסכמה pairwise 65% (win-vs-tie); cross-check ציטוטים: 186 מסכימים / 4 "לא בגרף עכשיו" / 3 בסנאפשוט האחזור (קהילות שנמחקו) / 1 טעות שופט; מכנים לכל אסטרטגיה מודפסים; סעיף אינקרמנטלי בשורות; שני הסעיפים הפרשניים של המתכנן (`## מתי לא הייתי משתמש בגרף כאן`, `## מה הייתי משנה`) נשמרים בין regen; סקירה: Task 4 accept, 3+5 fix-required → תוקן (c2ce809, 0977025, c313db4, 55598fe); **פערים מתועדים:** 0/32 שאלות truth, 13 הקשרים נשמטו (drift), Leiden לא אינקרמנטלי |
...
### סקירת Plan 3 Tasks 3–5 (2026-09-17, brain-reviewer) — Task 3 fix-required, Task 4 accept, Task 5 fix-required
- **שוחזר בדיוק:** ממוצעי השופט מ-69 קובצי out (correctness agentic 1.395 · s5 1.0 · s4 0.95 · s6 0.667 · s1r 0.517 · s1 0.5 · s2 0.5 · s3 0.469); blinding מבני מחזיק (0 דליפות ב-143 קבצים); A/B אקראי (61/120); חפיפה 20.0%; 3 הישויות המשוחזרות נושאות provenance של Plan 1 + הפרוסה; rollback 26; COMPLETE.
- 🔴 `pairwise_table` סופר verdicts (143) ולא זוגות (120) → 23 זוגות חופפים במשקל כפול: s4 0.625→**0.688**, s6 0.30→0.333, s5 0.857→0.833; **הסכמה pairwise 15/23 = 65%** (כל אי-הסכמה win-vs-tie) לא פורסמה.
- 🔴 תהליך: ה-snapshot לא הוקפא — האינקרמנטלי שינה את הגרף 09:30–09:53 בזמן חלון המדידה; 13 הקשרים נשמטו (s1 −6, s1r −3 — זרוע ה-baseline נפגעה הכי הרבה), 3 ציטוטי קהילה הפכו "לא בגרף".
- 🟠 בודק הציטוטים ללא label `StatusChange` וכל 40-hex נבדק רק כ-Commit → 11 מ-14 "לא בגרף" קיימים (10 StatusChange + Chunk.id בלי prefix); רק cq06.s4 היא טעות שופט אמיתית.
- 🟠 מכנים לא שווים בין אסטרטגיות (s1 n=26 מול s3 n=32) לא מודפסים; "184 מתוך 184" מול 190 נשפטו; Δ-עלות בטבלת "מתי מה" שייך למובילת ה-recall ומודפס בשורת מובילת ה-correctness; סעיף האינקרמנטלי מדפיס 645 hashes בתא אחד; MANIFEST של השופט יושב בתוך עץ ה-batches ושם את ה-baseline; gold של 4 שאלות temporal = פלט אותה פונקציה שהאסטרטגיה מריצה (לא מוצהר); הסתירה recall≠correctness קיימת רק בפרוזה של המתכנן, לא כשורה נגזרת.
- 🟡 `_best` בלי n מינימלי; chunks באינקרמנטלי מ-`{}`; "אין מספר ביד" לא מדויק (notes של מהנדס); עמודת strict לציטוטים; `ungraded_merges` 1656 מול 1657; `db.awaitIndexes` ב-`extract/graph.py`; rollback `--yes` לא הורץ (רק dry-run).
- **הכרעות:** (1) pairwise לפי `pair_id`; verdict מפוצל = tie; לפרסם הסכמה pairwise; (2) בדיקת ציטוטים: `StatusChange` + Chunk.id ל-40-hex, ולדווח `in_graph_now` ו-`in_retrieval_snapshot` (מזהים שהיו בהקשר של האחזור) בנפרד — הגרף זז, המדידה לא נזרקת; (3) מכנים ורשימת ההשמטות בדוח; (4) "מתי מה": Δ-עלות מיוחס במפורש למובילת ה-recall, "—" למובילת correctness בלי עלות, n ליד כל מוביל, שורה נגזרת "recall leader ≠ correctness leader"; (5) 4 שאלות temporal מסומנות "gold מאותו code path"; (6) MANIFEST של השופט מחוץ לעץ; (7) rollback: הקריטריון מתוקן ל"מוכח ב-`_ResetTest` עם fixture של slice", לא הרצה על הגרף האמיתי; (8) `make smoke` שקט + gate בסוף ע"י המתכנן.
- **לקחים (למוסכמות):** dedupe הוא per-aggregate; הסכמת שופטים תלויה בסולם — "מי ניצח" הכי פחות אמין; בודק ציטוטים רחב רק כרשימת ה-labels שלו; snapshot = census של גרף, לא רק sha של קוד.
...
- Plan 3 Tasks 3–5: `docs/lessons/16-judge-and-incremental.md`
