...
| 3 תשובות + שופט עיוור | 🟡 תשובות מצב A ✅ (4 answer-writers, 184/184, 0 נדחו); מצב B 19 הועברו; שופטים טרם פוזרו | 4da025c | סירובים "לא בהקשר" לפי אסטרטגיה: s1 11 · s1r 12 · s2 11 · **s3 16** · s4 7 · s5 0 · s6 9 — S3 מסרב הכי הרבה (הקשר של ישויות בלי הפריט עצמו), S4 הכי פחות; "ציטוט בהקשר": s4 72.9% / s5 86.7% / s6 97.3% מול 100% — מזהי Row יושבים ב-snippet ולא ב-key → תיקון הגדרה (ids מוכרים בתוך snippet נספרים) לפני השופט; q004.s4 הקשר ריק (אחזור ריק לגיטימי, נספר); הקשרים זהים לשאלות שונות ב-S4 (סט ברירת-מחדל של הדוגמה) — נרשם |
