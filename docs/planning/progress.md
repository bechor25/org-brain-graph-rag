...
| 4 אינקרמנטלי | 🟡 חלק 2 רץ עד הסוף (81a2010, 8e3957a); **באג נמצא**: `extract merge` חלקי דורס provenance של ישויות קיימות (3 ישויות בסיס איבדו את batch/evidence של Plan 1; rollback היה מוחק אותן) → תיקון: union + batch id עם prefix של slice; מתקנים ומשחזרים לפני כל reset | b5a0dba…8e3957a | **39.7s** לכל 12 השלבים; merge 0.9s (29 ישויות/6 יחסים, 0 נדחו); resolve 2.4s (1 מ-4 זהויות חדשות מוזגה); **communities 5.1s — Leiden הוא re-partition גלובלי: +0.3% צמתים → 645/1,297 member_hash השתנו, 1,297→1,066 קהילות, 62 דוחות אבדו, 124 נשמרו ע"י carry (42 בהעתקה בין רמות), 33 קהילות ≥25 בלי דוח, ההיררכיה השתטחה (533=533)** — נשאר כפי שהוא, נמדד; stamp-slice 13,846→0 null; index 0.9s (58,560 צמתים); **5 שאלות מצב A: 3/5** — inc2/inc5 בלי מפתח → S4 מהבנק שכולו `NOT status IN (Resolved…)` ו-9/10 החדשים Resolved = הבנק עיוור לפרוסה; מצב B רץ; rollback dry-run: 43 chunks, 10 WorkItem, 59 StatusChange, 3 Person, 29→26 ישויות אחרי התיקון |
| 5 דוח + שער | ⬜ | | |
