...
| 4 אינקרמנטלי | 🟡 חלק 1 ✅ (b5a0dba, ab99811): harvest→canon→load→chunk→extract build על הגרף החי; kg-extractor רץ; חלק 2 (merge→resolve→communities→index→5 שאלות) אחריו | b5a0dba | 10 issues של 2026 (KAFKA-20035…20065, 433 קיימים ב-Jira); **29.6s** לכל הצינור עד extract build (harvest 1.8s · canon 10.4s · load 8.6s · chunk 7.6s incl. 71 embeddings ב-5.2s · build 1.3s); +73 צמתים / +168 קשתות (StatusChange 59, Person 4) / +71 chunks; census 58,622→58,766; **בסיס byte-identical: 5/5 sha1** (שורות `slice != incremental`); idempotency: canon/load/chunk/build ריצה שנייה 0; batch אחד, 7 chunks (2 issues קצרים מ-300 תווים ו-Test בלי KIP הוחרגו); פתוח לחלק 2: `extract merge --slice`, `chunk --stamp-slice` (13,846 בסיס בלי slice), digest test על שורות בסיס |
| 5 דוח + שער | ⬜ | | |
