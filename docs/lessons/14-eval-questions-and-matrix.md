...
**1. gold שנגזר מהגרף הוא עובדה; gold שנכתב מזיכרון הוא דעה על הכותב.** 19 שאלות הכשירות קיבלו gold *בקוד*, עם `gold_query` על כל שורה — ולכן כשהתגלה ש-`changes_between` קרא בלי `ORDER BY`, ראינו שה-gold של cq13/cq19 מדד את סדר האחסון של Neo4j (recall 0.83 מקרי → 0.0 מסודר). gold ידני לעולם לא היה חושף את זה. הלקח לארגון: כל gold צריך את השאילתה שיצרה אותו, ואוסר כל קריאה בלי סדר טוטאלי (עכשיו נאכף ב-`_read`).

**2. "8 לכל סוג" הייתה החלטה שלי, והחשבון הפריך אותה.** 5 סוגים × 8 = 40 > 32. הסוכן לא "עיגל" — הוא דיווח `goal_reachable: false` וחילק 7/7/6/6/6. וכשה-merge אמר `complete: true` מוקדם מדי, התברר שאף בדיקה לא הסתכלה על ה-gold. שני מקרים של אותו כלל: קריטריון שלא נבדק בקוד לא קיים.

**3. שני recall, ולא אחד.** S1 מחזיר chunks; gold מורכב ממפתחות צמתים. בלי "chunk שה-parent שלו הוא צומת gold נספר", S1 היה מקבל 0.014 במקום 0.077 — והגרף היה "מנצח" בגלל הגדרה, לא בגלל אחזור. ה-leak guard של הקורס עובד לשני הכיוונים: גם לא לתת ל-baseline יתרון, גם לא לגזול ממנו.

**4. המטריצה הראשונה — מה שהיא אומרת ומה שלא.** S3 מוביל (recall 0.23, precision 0.56) כי הוא מחזיר מפתחות, לא טקסט; S6 0.29 על 18 שאלות שנקשרו; S4 15 ms ב-1.3k tokens. S1/S2 שורפים 3.8k tokens ב-precision 0.1. **אבל:** זו שכבה 2 — "האם הראיה הוחזרה". האם הסוכן *ענה נכון* מהראיה — זו שכבה 3, ואסור לקרוא את הטבלה הזאת כ"S3 עונה הכי טוב".

**5. reranker: המספר שנראה כמו ניצחון.** 19/19 top-1 השתנה — כי ציוני RRF יושבים במרווח 0.002. מה שהשתנה בפועל: hit@1 0.125→0.156, recall זהה, ×13 latency. בלי מדידה בשכבות זה היה "שיפרנו את הדירוג".

**6. cross-lingual: הגרף נושא את זה, לא ה-embedding.** S3/S4/S6 אינווריאנטים לשפה (Jaccard 1.0) כי הם מעוגנים במפתח; S1/S2 סוטים. זה מאשר את הממצא מ-Plan 2 עם 32 שאלות במקום 4.

**7. מה לא נמדד — במפורש.** 0/32 שאלות על בסיס ה-truth הסינתטי (המסלולים נארזו כ-spare ולא נשאלו). זרוע שלמה של ההכרעה לא מומשה. הדוח יגיד את זה; supplement אחרי השופט.
